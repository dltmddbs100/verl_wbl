...
BASE_URL = "http://vllm-genrm-server-0.vllm-genrm-server.p-ncai-wbl.svc.cluster.local:11111"
API_KEY = "EMPTY"
MAX_RETRIES = 5
BASE_DELAY = 2
MAX_WORKERS = 32
...
def get_response(problem, solution_str, ground_truth):
    # TODO: Think 스텝 분리해서 answer 스텝만 넘기도록 수정
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            data = {"model": MODEL_NAME, "messages": messages, 'max_tokens': 512, 
                    'temperature': 0.6, 'top_p': 0.95, 'top_k': 20}
            output = requests.post(chat_url, headers=headers, json=data, timeout=300)
